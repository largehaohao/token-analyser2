...
export default function Home() {
  ...
  function renderOverview() {
    ...
    const hasOverviewCalls = overviewAnalysis.calls.length > 0;
    const pricingCoverage = hasOverviewCalls
    ...
    <section className="trust-strip" aria-label="分析可信度摘要">
      <div className="trust-item">
        <span>计价覆盖</span>
        <strong>{hasOverviewCalls ? pricedCalls + '/' + overviewAnalysis.calls.length + ' 调用' : '暂无调用'}</strong>
